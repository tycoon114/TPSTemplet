...
public class AnimationManager : MonoBehaviour
{
    ...
    void _AsyncAnimation(int animTrack, AnimationReferenceAsset animClip, bool loop, float timeScale, float mixDuration) 
    {
            ...
            case AnimState.Idle:
                switch (_PrevState) {
                    case AnimState.Pet:
                        ...
                        _AsyncAnimation(1,AnimClip[3],false,1f,animationMix);
                        _AsyncAnimation(2,AnimClip[4],false,1f,animationMix);
                        break;
                    case AnimState.Look:
                        eyePointTransform.localPosition = Vector3.SmoothDamp(eyePointTransform.localPosition, Vector3.zero, ref velocity, 0.1f);
                        _AsyncAnimation(1,AnimClip[7],false,1f,animationMix);
                        _AsyncAnimation(2,AnimClip[8],false,1f,animationMix);
                        break;
                    default:
                        break;
                }
                break;
            case AnimState.Pet:
                _AsyncAnimation(1,AnimClip[1],true,1f,animationMix);
                _AsyncAnimation(2,AnimClip[2],true,1f,animationMix);
                break;
            case AnimState.Look:
                _AsyncAnimation(1,AnimClip[5],true,1f,animationMix);
                _AsyncAnimation(2,AnimClip[6],true,1f,animationMix);
                break;
        }
    }
}
